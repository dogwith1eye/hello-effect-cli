 ...
 */
export const program1 = Z.gen(function* ($) {
  const foo = yield* $(Z.service(Foo));
  yield* $(Z.logInfo(`program1 ${JSON.stringify(foo)}`));
});

export const program2 = Z.gen(function* ($) {
...
export const FooLive = ZL.succeed(Foo)({ foo: 4 });
export const BarLive = ZL.succeed(Bar)({ bar: 2 });
