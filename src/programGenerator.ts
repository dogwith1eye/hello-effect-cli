...
export const programHell: hell = pipe(
...
export const programPurgatory: purgatory = pipe(
...
export const programParadise: paradise = Z.gen(function* ($) {
...
export const programParadiseErr: paradiseErr = Z.gen(function* ($) {
  const random = yield* $(Z.service(CustomRandom));
  const foo = yield* $(Z.service(Foo));
  const bar = yield* $(Z.service(Bar));
